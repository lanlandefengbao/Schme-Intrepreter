...
""" 
Procedures could be either with normal operating logic(evaluated all operands and put them into operator to do the calculation)
OR with special logics of evaluation, which we need to explicitly define.
"""
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.define(signature, scheme_eval(expr.first, env))
        return signature
    elif isinstance(signature, Pair) and scheme_symbolp(signature.first):
        func = signature.first
        formals = signature.rest
        if isinstance(formals, Pair):
            validate_formals(formals)
        body = expressions.rest
        env.define(func, LambdaProcedure(formals, body, env))
        return func
...
def do_lambda_form(expressions, env):
    ...
    validate_formals(formals)
    body = expressions.rest
    return LambdaProcedure(formals, body, env) # haven't verify that body is a valid function
